                                ...
                                <ul className='list list-inline mb-0'>
                                    ...
                                    {navData.length !== 0 && navData.map((item, index) => (
                                        <li className='list-inline-item' key={index}>
                                            <Button onClick={() => openDropdownFun(index)}>
                                                ...
                                                >
                                                    {item.cat_name} <KeyboardArrowDownIcon className={`${openDropdownMenu && openDropdownMenuIndex === index && 'rotateIcon'}`} />
                                                </a>
                                            </Button>
                                            {item.items.length !== 0 && (
                                                <div className={`dropdown_menu ${openDropdownMenu && openDropdownMenuIndex === index && 'open'}`}>
                                                    <ul>
                                    ...
                                    <li className='list-inline-item position-static'>
                                        <Button onClick={() => setOpenMegaMenu(!openMegaMenu)}>
                                            <Link>Shop <KeyboardArrowDownIcon className={`${openMegaMenu && 'rotateIcon'}`} /></Link>
                                        </Button>
                                                        ...
                                                        {item.items.length !== 0 && (
                                                            <ul className='mt-4 mb-0'>
                                                                {item.items.map((item_, index) => (
                                                                    <li key={index}>
                                                                        <Link onClick={props.closeNav} to={`/cat/${item.cat_name.toLowerCase()}/${item_.cat_name.replace(/\s/g, '-').toLowerCase()}`}>
                                                                            {item_.cat_name}
                                                                        </Link>
                                                                    </li>
                                                                ))}
